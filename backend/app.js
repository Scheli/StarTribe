...
function pickRandomCard() {
  ...
  // Simple, safe sanitizer middleware that only mutates req.body and req.params
  // (avoids touching req.query which may be a getter-only property in some setups)
  function simpleSanitize(obj) {
    ...
    for (const key of Object.keys(obj)) {
      ...
      if (val && typeof val === 'object') simpleSanitize(val);
    }
  }

  app.use((req, res, next) => {
    try {
      if (req.body) simpleSanitize(req.body);
      if (req.params) simpleSanitize(req.params);
      // intentionally do NOT modify req.query to avoid setter/getter issues
    } catch (e) {
      console.warn('simpleSanitize error', e);
    }
    next();
  });
  ...
  // Basic rate limiter for auth endpoints (protect against brute-force)
...
// ---------------- sicurezza e utente ----------------
  ...
  // protect login with basic rate limiter
...
// Generic error handler: always return JSON to clients (prevents HTML error pages)
app.use((err, req, res, next) => {
  console.error('Unhandled server error:', err);
  if (res.headersSent) return next(err);
  res.status(err && err.status ? err.status : 500).json({
    success: false,
    message: process.env.NODE_ENV === 'production' ? 'Internal server error' : (err && err.message ? err.message : 'Internal server error')
  });
});
>>>>>>> Stashed changes
