...
app.get("/api/post", async (req, res) => {
  ...
  try {
    ...
    const postsFormatted = await Promise.all(posts.map(async (post) => {
      ...
      const likesArray = Array.isArray(post.likes) ? post.likes.map(l => l.toString()) : [];
      ...
      return {
        ...
        likedByMe: likesArray.includes(userId)
      };
    }));
...
app.post("/api/like", async (req, res) => {
  ...
  try {
    ...
    const db = await connectToDB();
    const postsColl = db.collection("articoli");
    const usersColl = db.collection("utenti");
    ...
    const result = await postsColl.updateOne(
    ...
    if (result.modifiedCount > 0) {
      ...
      try {
        ...
        console.warn("Impossibile aggiornare punti autore:", e);
      }
    }
    ...
    res.json({ success: true, message: "Like aggiunto con successo", likesCount });
  } catch (err) {
...
app.post("/api/unlike", async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: "Like rimosso con successo", likesCount });
  } catch (err) {
