...
export async function initBackground(engine){
  ...
  const rInit = THREE.MathUtils.clamp(unitRadius(mars) * CAMERA.RADIUS_MULT, CAMERA.MIN_DIST, CAMERA.MAX_DIST);
  orbit.setTarget(mars); orbit.setRadius(rInit); orbit.setSpeed(0.11); orbit.setElevation(0.22);
  let startOrbitAfterFocus = true;

  // ——— Input ———
  const onKey = (e)=>{
    if (e.key === "0" && mars){
      const dist = Math.max(CAMERA.MIN_DIST, unitRadius(mars) * CAMERA.RADIUS_MULT);
      smoothFocusTo(mars, { mult:CAMERA.RADIUS_MULT, minDist:dist, dur:1.0 });
